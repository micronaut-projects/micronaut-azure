...
snippet::example.JwtSigner[project-base="doc-examples/example", indent="0", source="main", tags="class"]

<1> The `KeyVaultKeySigner` bean is injected
<2> The payload is signed with the configured default algorithm

If you need to override the algorithm per call, use the overloaded method:

snippet::example.JwtSigner[project-base="doc-examples/example", indent="0", source="main", tags="algorithm"]
