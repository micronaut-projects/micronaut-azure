...
dependency:io.micronaut.azure:micronaut-azure-function-http[]
...
snippet::example.MyHttpFunction[project-base="doc-examples/example", indent="0", source="main"]
...
[.lang-python]
WARNING: A Python class cannot subclass api:azure.function.http.AzureHttpFunction[]; the Python example above delegates to an `AzureHttpFunction` instance instead, which starts the Micronaut application context. Because the Azure Functions runtime instantiates the function class before that application context, and with it the Python runtime, exists, the function class deployed as the Azure Function entry point must be written in Java, Kotlin or Groovy; the controllers it routes to can be written in Python.
