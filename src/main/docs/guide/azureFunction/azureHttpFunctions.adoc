...
[.lang-python]
WARNING: The Azure Functions runtime instantiates the function class before the Micronaut application context, and with it the Python runtime, exists, and reads the `@HttpTrigger` annotation of the parameter, which is not emitted for Python classes. The function class deployed as the Azure Function entry point must therefore be written in Java, Kotlin or Groovy; the controllers it routes to can be written in Python, and a Python function class like the one above can be used from a running application (for example in a test).
