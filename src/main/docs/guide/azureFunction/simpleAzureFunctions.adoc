...
[.lang-python]
WARNING: The Azure Functions runtime instantiates the function class itself, before any Micronaut application context, and with it the Python runtime, exists, and a Python class cannot subclass api:azure.function.AzureFunction[], which starts the application context in its constructor. A Python function class like the one above is therefore only usable as a bean of a running application (for example in a test); the class deployed as the Azure Function entry point must be written in Java, Kotlin or Groovy.
